...
func (r *Repo) Create(ctx context.Context, d *Device) (int64, error) {
	sql := `INSERT INTO device (user_id, name, room, type, status, metadata)
	...
	err := r.q.QueryRow(ctx, sql, d.UserID, d.Name, d.Room, d.Type, status, d.Metadata).Scan(&id)
	return id, err
}

// GetByID returns a device by ID.
func (r *Repo) GetByID(ctx context.Context, id int64) (*Device, error) {
	sql := `SELECT id, user_id, name, room, type, status, metadata, created_at, last_seen
			FROM device WHERE id = $1`
	var d Device
	err := r.q.QueryRow(ctx, sql, id).Scan(
		&d.ID, &d.UserID, &d.Name, &d.Room, &d.Type, &d.Status, &d.Metadata, &d.CreatedAt, &d.LastSeen,
	)
	if err != nil {
		return nil, err
	}
...
func (r *Repo) ListByUser(ctx context.Context, userID int64) ([]Device, error) {
	...
	for rws.Next() {
		var d Device
		if err := rws.Scan(&d.ID, &d.UserID, &d.Name, &d.Room, &d.Type, &d.Status, &d.Metadata, &d.CreatedAt, &d.LastSeen); err != nil {
			return nil, err
		}
		out = append(out, d)
	}
...
func (r *Repo) Update(ctx context.Context, userID, deviceID int64, req *UpdateDeviceRequest) error {
	...
	return r.q.Exec(ctx, sql, deviceID, userID, req.Name, req.Room, req.Type, req.Status, req.Metadata)
}
